...
**Remote Authentication Dial-In User Service (RADIUS)** is implementing the AAA server when configuring enterprise authentication. Rather than storing and validating user credentials directly, it forwards data between the RADIUS server and the supplicant without reading it.
...
**A certificate of destruction** shows the make, model, and serial number of each drive that was handled, plus the date of destruction and how it is destroyed.
...
**User Account control (UAC)** is a Windows security feature designed to protect the system against malicious scripts and attacks that could exploit the powerful privileges assigned to members of the Administrative group.
...
**WEP** is an outdated encryption protocol that was retired in 2004. WEP used a static RC4 key to encrypt data over the network.
**WPA3** uses passphrase-based group authentication of stations in private mode; it changes the method this secret is used to agree with session keys.
...
**File History** is a Windows feature for automatically backing up user data.
